...
function getUrl (url) {
  ...
  https.get(url, function (res) {
    ...
    res.on('end', function () {
      // 由于获取到的数据是JSON格式的，所以需要JSON.parse方法浅解析
...
function savedImg (imageList, title) {
  ...
  console.time('imageList=' + imageList);
  if (imageList) {
    imageList.forEach(function (item, index) {
      let imgTitle = index;
      let imgFilename = imgTitle + '.jpg';
      let imgSrc = 'http://' + item.substring(9);
      console.log('======================src=', imgSrc);
      request({ uri: imgSrc, encoding: 'binary' }, function (err, res, body) {
        if (!err && res.statusCode === 200) {
          fs.writeFile('./image/' + title + '/' + imgFilename, body, 'binary', function (err) {
            if (err) {
              console.log(err);
            }
          });
